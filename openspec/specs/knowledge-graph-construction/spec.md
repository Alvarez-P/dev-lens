...
> **Note (implementation)**: Edge derivation lives in `SemanticModelBuilder`; `GraphBuilder` resolves FQNs → deterministic UUIDs and enforces integrity. All 6 derivation rules in this table are satisfied behaviorally (verified W2/W3).
...
> **Note (implementation)**: Deterministic UUIDs are derived via `reconstitute()` (SHA-1 seeded by `repo:version:fqn`), satisfying byte-identical re-runs.
