...
> **Note (implementation)**: FKs exist in migration DDL (`onDelete: CASCADE`); no `@ManyToOne`/`@JoinColumn` on the entity — acceptable since nodes are soft-deprecated, never hard-deleted (verified W7).
...
> **Note (implementation)**: The transaction is all-or-nothing — a failed build leaves zero rows (per the rollback scenario). `snapshot.fail()` transitions the in-memory aggregate and a `GraphBuildFailedEvent` is dispatched; BullMQ retries then DLQ handle recovery (verified S4).
