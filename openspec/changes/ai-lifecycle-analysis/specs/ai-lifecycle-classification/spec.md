...
Framework detection SHALL parse manifests deterministically (`package.json`, `pom.xml`, `requirements.txt`, `pyproject.toml`) to produce candidate frameworks, then SHALL request the LLM to confirm/refine candidates using entry-point files, returning `{ framework, architecture, confidence }`.
...
Every classified item SHALL carry a `confidence`. Items below threshold SHALL emit `Unknown`/needs-review instead of guessing (RFC-007 §6.4 honesty).
