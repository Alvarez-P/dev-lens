...
| ADR-2 | Marker map as a deterministic constant in the analysis module (**`package.json` only**); LLM output authoritative | vs. config-driven markers in `ai.frameworks/*.json` (analysis→AI dependency, wrong direction) | RFC-001 deterministic-first; LLM confirms `{framework, architecture, confidence}`; no manifest → `unknown`/`confidence:0`, never guessed. Non-TS markers deferred to `parser-abstraction` |
