...
function buildFixtureIr(includeRepository: boolean): IrProject {
  const classes: {
    name: string;
    role: string;
    endpoints: { name: string; httpMethod: string; path: string; parameters: string[] }[];
  }[] = [
    {
    ...
    { name: 'UsersService', role: 'service', endpoints: [] },
  ];

  if (includeRepository) {
    classes.push({ name: 'UsersRepository', role: 'repository', endpoints: [] });
  }
...
describe('Knowledge Graph Incremental Build (E2E)', () => {
  ...
  beforeEach(async () => {
    ...
    for (const eventType of [
      ...
      dispatcher.registerHandler(eventType, (event): Promise<void> => {
        const graphEvent = event as unknown as { eventType: string; analysisId: string };
  ...
  function seedAnalysis(analysisId: string, includeRepository: boolean): void {
    const analysis = Analysis.reconstitute(
      AnalysisId.from(analysisId),
      SnapshotId.from('snap-1'),
      RepositoryId.from('repo-1'),
      AnalysisStatus.COMPLETED,
      buildFixtureIr(includeRepository),
      {},
      null,
      new Date(),
      new Date(),
    );
    void analysisRepository.save(analysis);
  }

  it('should soft-deprecate removed nodes, bump the version, and dispatch an updated event on re-analysis', async () => {
    const firstAnalysisId = '11111111-2222-3333-4444-555555555551';
    const secondAnalysisId = '11111111-2222-3333-4444-555555555552';
    seedAnalysis(firstAnalysisId, true);
    seedAnalysis(secondAnalysisId, false);

    await service.buildGraph(firstAnalysisId);
    ...
    expect(dispatchedEvents.map((event) => event.eventType)).toEqual([
      'knowledge-graph.built',
      'knowledge-graph.updated',
    ]);
    expect(dispatchedEvents[1].analysisId).toBe(secondAnalysisId);
  });
});
